...
<div class="app-shell">
    ...
    <main class="main-content">
        ...
        <div class="page-view leader-page">
            ...
            <section class="session-history">@forelse ($sessions as $session)<article class="panel history-session"><div class="history-header"><div><span class="section-kicker">{{ $session->type }}@if(!$session->ended_at || $session->ended_at->isFuture()) <span class="live-dot" style="margin-left:6px">● Live</span>@endif</span><h2>{{ $session->name }}</h2><p class="muted">{{ $session->started_at->format('D, d M Y · h:i A') }} · {{ $session->location ?: 'Main campus' }} · {{ $session->durationForHumans() }} · <strong>Set by {{ $session->leader?->name ?: 'Unknown leader' }}</strong></p></div><div class="history-total"><strong>{{ $session->records_count }}</strong><span>attended</span></div></div><div class="history-attendees">@forelse ($session->records as $record)<span class="attendee-chip">{{ $record->member->name }}<small>{{ $record->checked_in_at->format('h:i A') }}</small></span>@empty<p class="muted">No members attended.</p>@endforelse</div></article>@empty<section class="panel empty-history"><h2>No session history yet.</h2><p class="muted">Your completed gatherings will appear here.</p></section>@endforelse</section>
