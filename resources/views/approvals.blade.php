...
<body>
    <main class="page-view" style="max-width:900px;margin:auto;min-height:100vh">
        ...
        <p class="eyebrow">Super admin</p><h1>Leader approvals</h1><p class="muted" style="margin:12px 0 35px">Review new accounts before they can enter the attendance workspace.</p>
        ...
        <section class="panel" style="padding:24px">
            @forelse ($pendingUsers as $pendingUser)
                <div style="display:flex;justify-content:space-between;align-items:center;gap:15px;border-bottom:1px solid #e7e7e3;padding:17px 0"><div><strong>{{ $pendingUser->name }}</strong><p class="muted" style="font-size:11px;margin-top:4px">{{ $pendingUser->email }} · Registered {{ $pendingUser->created_at->diffForHumans() }}</p></div><form action="{{ route('admin.approvals.approve', $pendingUser) }}" method="POST">@csrf<button class="button button-dark" type="submit">Approve leader</button></form></div>
            @empty
                <p class="muted">No leader accounts are waiting for approval.</p>
            @endforelse
        </section>
    </main>
</body>
